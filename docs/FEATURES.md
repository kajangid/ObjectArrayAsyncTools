...
Complete API documentation, type signatures, parameter specifications, and code examples for all 13 tools, the compile-time `VERSION` constant, and CLI commands in `@kjangid/array-async-tools`.
...
function groupByMap<T, K>(items: readonly T[], keySelector: ((item: T, index: number) => K) | keyof T): Map<K, T[]>;
```
...
```typescript
function retry<T>(fn: (context: RetryContext) => Promise<T> | T, options?: RetryOptions): Promise<T>;
```
...
```typescript
import { VERSION } from "@kjangid/array-async-tools";
