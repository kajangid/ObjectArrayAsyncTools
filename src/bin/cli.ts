...
export function printHelp(): void {
  const text = `
@kjangid/array-async-tools CLI (v${VERSION})
...
function resolveJsonInput(fileArg: string | undefined, stdinContent?: string): unknown {
  let content = "";
  if (fileArg && fileArg !== "-") {
    try {
 ...
 */
export async function runCli(argv = process.argv.slice(2), stdinOverride?: string): Promise<number> {
  ...
  try {
    switch (command) {
      ...
      case "timeout": {
        ...
        await promiseTimeout(() => executeChildProcess(commandArgs[0], commandArgs.slice(1)), ms);
        return 0;
      }
